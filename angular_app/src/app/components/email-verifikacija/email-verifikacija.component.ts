...
export class EmailVerifikacijaComponent {
  ...
  onSubmit() {
    this.mojConfig.verifikacijaEmaila(this.verifikacijaEmail()).subscribe({
      next: (res: any) => {
        ...
        if (this.prethodnaAkcija === "registracija")
          ...
          this.userStore.getRole().subscribe((r) => {
            let tipKorisnika = r
            if (tipKorisnika == 'Poslodavac') { this.router.navigate(['poslodavac']); }
            else { this.router.navigate(['aplikant']); }
            alert("Uspješno ste logirani.")
          })
      ...
      error: (err: any) => {
        alert("Pogrešan kod. Molimo Vas pokušajte ponovo.")
      }
    });
  }
  goToNovaLozinka() {
